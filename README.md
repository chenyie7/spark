...
| 📈 性能可观测 | 自动采集 Token/耗时/P0 收敛数据，支持跨运行对比 |
...
│   ├── dump-agent-payload.sh     #   PostToolUse hook：采集性能数据
│   ├── synthesize-benchmark.sh   #   Stop hook：合成报告
│   ├── schema.py                 #   性能日志合成引擎
│   └── compare.py                #   跨运行对比工具
│
└── docs/                         # 项目文档
    └── BENCHMARK.md              #   基准测试系统详解
```
...
每次 `/build` 通过 Claude Code Hooks 自动采集性能数据，产出结构化的 JSON + Markdown 报告。详见 [基准测试文档](docs/BENCHMARK.md)。
...
项目内置了完整的流水线性能基准测试系统，每次 `/build` 自动采集 Token 消耗、耗时、P0 收敛曲线、修复效率等数据，支持跨运行横向对比和异常检测。

详见 **[基准测试文档](docs/BENCHMARK.md)**。
