...
整个流水线在 **一个 Claude Code 会话** 内运行。`/build` Skill 作为调度 Agent，通过 `Agent` 工具按需创建 coder 和 reviewer 子 Agent。各子 Agent 共享同一会话的文件系统和上下文，与现有 `/review` Skill 的工作方式一致。

```
┌──────────────────────────────────────────┐
│         当前 Claude Code 会话              │
│                                          │
│   /build Skill（调度 Agent）               │
│       │          │                       │
│       │ Agent    │ Agent                 │
│       │ 工具     │ 工具                   │
│       ▼          ▼                       │
...
对比方案 2（Python CLI 调度器）才是启动多个独立的 Claude Code 进程。Skill 方案胜在轻量和即时可用。
